...
{
	LPCWSTR szCertFileName = L"private.pfx";

	CRYPT_DATA_BLOB pPFX;
	LPCWSTR szPassword = L"testpassword";
	HCRYPTPROV hCPContext = NULL;

	LPCWSTR szCertName = L"CN=TestName";
	CRYPT_DATA_BLOB pPfxOutputBlob = { 0, NULL };
	LPCWSTR szOutFile = L"private_extended.pfx";

	PCRYPT_KEY_PROV_INFO provInfo;

	//Uncomment if you want to run this via command line.
	...
	char buffer[1024];
	printf("Created File: %s\\%S", _getcwd(buffer, sizeof(buffer)), szOutFile);
	while (!_kbhit());
